...
- [ ] **Experiment — Momarchy Tablet (ARM):** investigate the Huawei MediaPad T3 10 (AGS-L09) as a secondary native ARM64 deployment target: first prove the existing postmarketOS/lk2nd path and real hardware support (display, touch, GPU/Wayland, Wi-Fi), then evaluate Hyprland plus an Arch Linux ARM / minimal Omarchy-Momarchy userspace; keep the MBP13 as the primary reference target until the tablet path is proven.
